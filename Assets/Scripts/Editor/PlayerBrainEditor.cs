...
public class PlayerBrainEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        // If a PlayerMinionCommander lives on the same or child object, show its bindings too.
        var commander = brain.GetComponentInChildren<PlayerMinionCommander>();
        if (commander != null)
        {
...
[CustomEditor(typeof(PlayerMinionCommander))]
public class PlayerMinionCommanderEditor : Editor
{
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();

        var commander = (PlayerMinionCommander)target;
    ...
    private string JoinBindingsForGroups(InputAction action, params string[] groups)
    {
        ...
        string Human(InputBinding b) =>
            InputControlPath.ToHumanReadableString(b.effectivePath ?? b.path, InputControlPath.HumanReadableStringOptions.OmitDevice);
        ...
        for (int i = 0; i < action.bindings.Count; i++)
        {
            ...
            if (b.isComposite)
            {
                for (int j = i + 1; j < action.bindings.Count && action.bindings[j].isPartOfComposite; j++)
                {
                    ...
                    if (!MatchesByHeuristic(part)) continue;
                    string partName = string.IsNullOrWhiteSpace(part.name) ? "part" : part.name;
                    string line = $"{partName}: {Human(part)}";
                    if (seen.Add(line)) lines.Add(line);
                }
                continue;
            }
